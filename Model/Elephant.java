public class Elephant extends Piece {
  public Elephant(int x_init, int y_init, String col_set) {
    x = x_init;
    y = y_init;
    colour = col_set;
  }
  ...
  bool valid_move(int x_dest, int y_dest) {
    ...
    if (0 <= x_dest <= 8 && y_min <= y_dest <= y_max) {
     if (is_occupied(x_dest,y_dest) != colour) {
       int x_length = x - x_dest;
       int y_length = y - y_dest;
       if (abs(x_length) == 2 &&abs(y_length) == 2) {
         if (is_occupied(x + (x_length/2), y + (y_length/2)) == "free") {
           return true;
         }
       }
      }
